...
async function fetchEntityTableData() {
    await fetch("http://localhost:3040/" + `${server_address}/overall/table/data`, {
        method: "POST",
        headers: {
...
async function fetch_cooccurr_into(outlet, entity, co_occurr_entity) {
    await fetch("http://localhost:3040/" + `${server_address}/processed_data/cooccurr_info/grouped/${outlet}/${entity}/${co_occurr_entity}`)
        .then(res => res.json())
        .then(json => {
            const target_entity = {
                name: json.target,
...
async function fetchSelectedEntityHex(target: string, co_occurr_entity: string="") {
    ...
    const promiseArray: any[] = []
    promiseArray.push(new Promise((resolve) => {
        fetch("http://localhost:3040/" + `${server_address}/hexview/grouped/${target}/all`)
            .then(res => res.json())
            .then(json => {
...
async function fetch_entity_grouped_node(hex_candidates, outlet) {
    ...
    await fetch("http://localhost:3040/" + `${server_address}/processed_data/scatter_node/grouped/hex_candidates/${outlet}`, {
        method: "POST",
        headers: {
            "Accept": "application/json",
            "Content-Type": "application/json"
